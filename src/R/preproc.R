...
bmi_z<-bmi_orig %>% select(PATID,SITE,SEX,AGEMOS,AGEMOS_FUZZY,BMI) %>%
  left_join(read.csv("https://www.cdc.gov/growthcharts/data/zscore/bmiagerev.csv") %>%
              mutate(Sex = as.numeric(Sex),
                     ...
                     Agemos_FUZZY = floor(Agemos/10)*10) %>%
              filter(!is.na(Sex)) %>%
              select(Sex,Agemos,Agemos_FUZZY,L,M,S),
...
ht_z<-bmi_orig %>% select(PATID,SITE,SEX,AGEMOS,AGEMOS_FUZZY,HT) %>%
...
wt_z<-bmi_orig %>% select(PATID,SITE,SEX,AGEMOS,AGEMOS_FUZZY,WT) %>%
  ...
  mutate(WT_Z = case_when(L==0 ~ log(WT/M)/S,
                          TRUE ~ ((WT/M)^L-1)/(L*S)))
bmi<-bmi_orig %>%
  left_join(bmi_z %>% select(PATID,BMI_Z),by="PATID") %>%
  left_join(ht_z %>% select(PATID,HT_Z),by="PATID") %>%
  left_join(wt_z %>% select(PATID,WT_Z),by="PATID") %>%
  select(PATID,SITE,HT,WT,BMI,HT_Z,WT_Z,BMI_Z)

bmi_ind<-tbl1 %>% select(PATID,SITE) %>%
  left_join(bmi %>%
              select(PATID,SITE,HT,WT,BMI) %>%
...
lab_ind<-tbl1 %>% select(PATID,SITE) %>%
  left_join(lab %>%
              gather(var,val,-PATID,-SITE) %>%
              mutate(var = paste0(var,"_ind"),
                     val = as.numeric(!is.na(val))) %>%
...
endpt2<-endpt %>% 
  select(PATID,SITE,mesalazine,corticosteroids,immunomodulator, biologics, colectomy) %>%
  mutate(standard = pmin(mesalazine,corticosteroids,na.rm=T)) %>%
  select(PATID,SITE, standard,immunomodulator, biologics, colectomy) %>%
  gather(var,val,-PATID,-SITE) %>%
  filter(!is.na(val)) %>%
  group_by(PATID,SITE) %>%
  mutate(ord = paste0("trt",order(val))) %>%
  ungroup %>%
  select(PATID,SITE,var,ord) %>%
  spread(ord,var) %>%
